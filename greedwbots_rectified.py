...
qualequ_sound = pygame.mixer.Sound("sounds/greed_qualifying_answer_equal.wav")
def open_image(path):
...
def onKeyPress(event):
    ...
    if not (root.term_state == term.ready):
        pass
         ...
         pass
    elif (IgrokiDummy[player_term[0]]['isBot'] == 1 and IgrokiDummy[player_term[1]]['isBot'] == 1): #(bots_in_term == 2):
        pass
    else:
...
def after_otbor():
    global label_w_names
    root.after_cancel(root.komanda)
    texts = ('капитан', 'игрок 1', 'игрок 2', 'игрок 3', 'игрок 4', 'выбывает')
    for i in range(6):
        tk.messagebox.showinfo("", 'Кто ' + texts[i] + '?')
        if (i == 0):
            root.otbor_corr_field.place_forget()
            t_v_otbor.place_forget()
        if not (equal_prev[i]) and not (equal_next[i]):
            canvas.itemconfig(ans_t[i], state='normal')
            qualans_sound.play()
        elif (equal_prev[i] is False):
            for j in range(6):
                if root.XYZ[i]['Distance'] == root.XYZ[j]['Distance']:
                    canvas.itemconfig(ans_t[j], state='normal')
            qualequ_sound.play()
            root.title("Несколько игроков дали одинаково близкий к верному ответ")
            tk.messagebox.showinfo('', 'Кто ответил раньше?')
            canvas.itemconfig(ans_t[i], state='normal', fill="#ff3f00")
            qualans_sound.play()
        else:
            if equal_next is False:
                tk.messagebox.showinfo('', 'Кто ответил раньше?')
            canvas.itemconfig(ans_t[i], state='normal', fill="#ff3f00")
            qualans_sound.play()
        root.title(texts[i].capitalize() + " - " + root.XYZ[i]["Name"] + ", ответ: " + str(root.XYZ[i]["Otvet"]))
        tk.messagebox.showinfo(texts[i].capitalize() + " - " + root.XYZ[i]["Name"],
                               "Ответ: " + str(root.XYZ[i]["Otvet"]))
        if root.XYZ[i]['isBot'] == 1:
            h = '(бот)'
        else:
            h = ''
        if (i == 5):
            log.write(root.XYZ[i]["Name"] + " (" + str(root.XYZ[i]["Otvet"]) + ") " + h + "\n")
        else:
            log.write(root.XYZ[i]["Name"] + " (" + str(root.XYZ[i]["Otvet"]) + ") - " + texts[i] +' '+h + "\n")
        canvas.itemconfig(ans_t[i], state='hidden', fill="#ffff00")
        canvas.itemconfig(label_w_names[root.XYZ[i]['Original']], state="hidden")
...
def showcorr():
    ...
    global komanda, equal_next, equal_prev
    for i in range(6):
        if (i <=4) and (root.XYZ[i]['Distance'] == root.XYZ[i+1]['Distance']):
            equal_next.append(True)
        else:
            equal_next.append(False)
    for i in range(6):
        if (i >=1) and (root.XYZ[i]['Distance'] == root.XYZ[i-1]['Distance']):
            equal_prev.append(True)
        else:
            equal_prev.append(False)
    for i in range(6):
        #print(root.XYZ[i])
        xx, yy, xx1, yy1 = canvas.coords(qualifying_round_stands[root.XYZ[i]['Original']])
        h3 = canvas.create_text(xx + 44, yy - 22, text=str(root.XYZ[i]['Otvet']), fill="#ffff00", width=80, tags=(i, 'pl_ans'))
        ans_t.append(h3)
        canvas.itemconfig(ans_t[i], state="hidden")
...
def _showcorr():
    root.after_cancel(otbor_waitforcorr)
    tk.messagebox.showinfo("", str(CORRECT))
    #qual_corr.set(CORRECT)
    root.XYZ = sorted(Players, key = lambda i: (i['Distance'], i["Vremya"]))
    global komanda, equal_next, equal_prev
    for i in range(6):
        if (i <=4) and (root.XYZ[i]['Distance'] == root.XYZ[i+1]['Distance']):
...
def on_off_otbor(o):
    if (states[o] == 0):
        ...
        root.begin = datetime.now()
        root.otvetbota = root.after(randint(3500, 6000), botwait) #root.after(randint(35, 60), botwait)
